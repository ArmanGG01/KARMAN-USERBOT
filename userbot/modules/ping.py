...
absen = [
    "**Eh ada Arman Sayang Aku🥰**",
    "**Hadir ganteng** 🥵",
    "**Hadir Sayang Ahhhh** 😘",
    "**Hadir Kakak Ganteng** 😉",
    "**Lapor Bang Arman Saya Hadir** 😁",
    "**Hadir Bang Maap Abis liat Bokep** 😊",
]
...
async def _(arman):
    await arman.reply(random.choice(absen))
...
async def redis(pong):
    ...
    await pong.edit(
        f"**✠╼━━━━━━━━━━━❖━━━━━━━━━━━✠
        f"** ➥ 𝙺𝙾𝙽𝚃𝙾𝙻\n"
        f"** ➥ 𝙺𝙴𝙺𝚄𝙰𝚃𝙰𝙽 𝙺𝙾𝙽𝚃𝙾𝙻: `%sms`\n"
        f"** ➥ 𝙳𝚄𝚁𝙰𝚂𝙸 𝙺𝙾𝙽𝚃𝙾𝙻 𝙳𝙰𝙽 𝙿𝙴𝙿𝙴𝙺: `{uptime}🕛`" % (duration)
        f"**✠╼━━━━━━━━━━━❖━━━━━━━━━━━✠
    )
...
async def redis(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("**Mengecek Kontol...**")
    await pong.edit("**0% ▒▒▒▒▒▒▒▒▒▒**")
    await pong.edit("**20% ██▒▒▒▒▒▒▒▒**")
    await pong.edit("**40% ████▒▒▒▒▒▒**")
    await pong.edit("**60% ██████▒▒▒▒**")
